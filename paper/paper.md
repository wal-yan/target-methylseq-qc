...
The `target-methylseq-qc` pipeline builds upon the standardised pipeline template maintained by the nf-core community [@ewels_nf-core_2020] for Nextflow  pipelines as well as makes use of the `nf-core/modules` project to install modules for FastQC, MultiQC [@ewels_multiqc_2016], bedtools, Picard tools as well as Samtools [@danecek_twelve_2021] within the pipeline (\autoref{subway-map}).
