...
The target-methylseq-qc pipeline builds upon the standardised pipeline template maintained by the nf-core community [@ewels_nf-core_2020] for Nextflow  pipelines as well as makes use of the nf-core/modules project to install modules for FastQC, MultiQC [@ewels_multiqc_2016], Bedtools, Picard as well as Samtools [@danecek_twelve_2021] within the pipeline \autoref{subway-map}.
...
Whereas the `bed_filter` mode requires a different set of columns in the input samplesheet CSV file, as shown in \autoref{samplesheet-2}.
...
The behaviour of the pipeline can be controlled through the pipeline parameters which are divided into different groups such as (i) Execution Mode, (ii) Input/Output Options (iii) Reference Genome Options in addition to the generic parameters inherited from the nf-core template such as (i) Max job request options (ii) Generic options and (iii) Institutional config options. A complete list of the parameters specific to target-methylseq-qc pipeline is summarised in \autoref{parameters}
